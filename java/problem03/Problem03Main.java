...
public class Problem03Main {
	...
	public static void main(String[] args) {
		...
		RationalNumber total =
				new RationalNumber(Integer.parseInt(tokens.get(0).substring(0, tokens.get(0).indexOf("/"))),
		...
		for (int index = 1; index < tokens.size(); index++) {
			...
			if (i.length() > 1) {
				current =
						new RationalNumber(Integer.parseInt(i.substring(0, i.indexOf("/"))),
								Integer.parseInt(i.substring(i.indexOf("/") + 1)));
				if (currentOp.equals("+")) {
