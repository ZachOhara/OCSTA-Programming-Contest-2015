...
public class Problem12Alternate {
	...
	public static void main(String[] args) {
		...
		System.out.println(Problem12Alternate.reorder(expression));
	}
	...
	public static String reorder(String exp) {
		...
		if (exp.substring(3, 4).equals("(")) {
			firstSubExp = exp.substring(3, Problem12Alternate.findEndOfExpression(exp, 3) + 1);
		} else {
			firstSubExp = exp.substring(3, 4);
		}
		if (exp.substring(firstSubExp.length() + 4, firstSubExp.length() + 5).equals("(")) {
			secondSubExp =
					exp.substring(firstSubExp.length() + 4,
							Problem12Alternate.findEndOfExpression(exp, firstSubExp.length() + 4) + 1);
		} else {
			secondSubExp = exp.substring(firstSubExp.length() + 4, firstSubExp.length() + 5);
		}
		return "(" + Problem12Alternate.reorder(firstSubExp) + " " + operator + " "
				+ Problem12Alternate.reorder(secondSubExp) + ")";
	}
